DROP PROCEDURE IF EXISTS ATROX.proc_atrox_service_enable_version;
DO $do$ BEGIN RAISE NOTICE '<<<DROPPED PROCEDURE ATROX.proc_atrox_service_enable_version>>>'; END $do$;

DO $do$
BEGIN
    EXECUTE $sql$CREATE PROCEDURE ATROX.proc_atrox_service_enable_version(
    IN i_se_id BIGINT,
    IN i_sv_id BIGINT,
...
BEGIN
    ...
    BEGIN
        ...
        FROM ATROX.atrox_service_version sv
        WHERE sv.sv_id = i_sv_id
          AND sv.se_id = i_se_id
          AND sv.sv_deleted = FALSE;

        IF v_target_status IS NULL THEN
            RAISE EXCEPTION 'Target service version not found.';
        END IF;
        ...
        UPDATE ATROX.atrox_service_version
        SET
            vs_id = v_disabled,
        ...
        UPDATE ATROX.atrox_service
        SET
            se_current_version_id = i_sv_id,
            se_updated_at = CURRENT_TIMESTAMP,
            se_updated_by = i_updated_by
        WHERE se_id = i_se_id
          AND se_deleted = FALSE;
...
END;
$$;$sql$;
    RAISE NOTICE '<<<CREATED PROCEDURE ATROX.proc_atrox_service_enable_version>>>';
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE '<<<FAILED CREATING PROCEDURE ATROX.proc_atrox_service_enable_version>>>';
END
$do$;